...
class Anime:
    ...
    def download(self, torrentName, downloadLink, animeName):
        ...
        os.system(f"move \"{torrentName}\" \"DownloadedTorrents\\{animeName}\\{torrentName}\" > nul 2>&1")#------------------------------IMPORTS------------------------------
...
class Anime:
    """
    Class that defines anime by different attributes : name, quality, episodes.
    """
    numberOfAnime = 0
    ...
    def download(self, torrentName: str, downloadLink: str, animeName: str):
        ...
        Args:
            ...
            animeName (str): The name of the folder to create where the file will be saved (doesn't create it if already exists and move the file to the existing one).
        """
        os.system('mkdir DownloadedTorrents > nul 2>&1')
        os.system(f'mkdir \"DownloadedTorrents\\{animeName}\" > nul 2>&1')
    ...
    def transfer(self, magnet: str, torrentName: str):
...
"""
        ...
        while stillWantsAnime:
            ...
            animeToDownload.append(Anime(animeName, qualityChoice, animeBegin, animeEnd))
            os.system('cls')
        ...
        for anime in animeToDownload:
            ...
            # The number of downloaded episode of the currently checked anime (anime). Will be used to calculate percentage.
            foundAnimes = 0


            # For each episode from the anime...
            for i in range(anime.begin, anime.end + 1):
                ...
                for u in uploaders:
                    ...
                    foundTorrent = NyaaPy.Nyaa.search(keyword=f'[{u}] {anime.name} - {epValue} [{anime.quality}p]', category=1, subcategory=2, filters=2)
                    print(f'Checking: [{u}] {anime.name} - {epValue} [{anime.quality}p]')
                    ...
                    if len(foundTorrent) != 0:
                        ...
                        for t in foundTorrent:
                            if t['name'].lower().find(f'{anime.name} - {epValue}'.lower()) != -1:
                                torrent = t
                    ...
                    # [...] Else, the program alerts you that no torrent have been found with the corresponding name.
                        ...
                        print(f"No torrent found for the name: [{u}] {anime.name} - {epValue} [{anime.quality}p].\nEither it still hasn't aired or doesn't exist...\n")
                        
                        if f'{anime.name} - Episode {epValue}' not in missingTorrents and u == uploaders[-1]:
                            missingTorrents.append(f'{anime.name} - Episode {epValue}')
            ...
            if unexpectedEnd:
                percentage = '0' if str(round(foundAnimes*100/(int(epValue) - anime.begin + 1), 2)).strip('0').strip('.') == '' else str(round(foundAnimes*100/(int(epValue) - anime.begin + 1), 2)).strip('0').strip('.')

            else:
                percentage = '0' if str(round(foundAnimes*100/(anime.end - anime.begin + 1), 2)).strip('0').strip('.') == '' else str(round(foundAnimes*100/(anime.end - anime.begin + 1), 2)).strip('0').strip('.')
            
            # This notifies you that every torrent from an anime has been fully downloaded.
            toaster = win10toast.ToastNotifier()
            toaster.show_toast('Nyaa Auto-download', f'The anime {anime.name} has been {verbalBase} at {percentage}%!')
            percentageStock = []
